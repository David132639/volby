...
Nebude zde zkoumána umělá hranice pro zahrnutí strany do rozdělování mandátů, u nás známá jako \uv{pětiprocentní hranice}.\footnote{Pětiprocentní hranice ve skutečnosti není zcela přesný název.
...
Při volbě více poslanců by výsledky odpovídaly ještě přesněji, lze si všimnout že jednoho ani pět poslanců nelze \uv{férové} rozdělit mezi 5 stran tak aby odpovídala procenta zisku hlasů a zisku mandátu.
...
V následujících letech začínají státy západní Evropy postupně tento nový systém zavádět, zde již ale nedochází k takovým kontroverzím.
...
\item $\lbrace \rbrace$ je zaokrouhlení dle daného postupu
\item $p_{x}=\left\lbrace\dfrac{h_{x}}{Q}\right\rbrace$ kde $Q$ je reálné číslo zvolené tak, aby platilo: $$N=\sum_{i=1}^{n} p_{i}$$
\end{itemize}
$Q$ libovolně splňující podmínky vytvoří stejný výsledek.\autocite{GAL1}
Pro D'Hondtovu metodu je $\lbrace \rbrace$ dolní celá část, pro metodu Sainte-Lague je to zaokrouhlení dle aritmetických pravidel, pro Adamsovu metodu je to dolní celá část...
...
Tato kvóta se nazývá Droopova, a je definována jako první přirozené číslo větší než $\frac{H}{N+1}$ kde $H$ je součet všech hlasů a $N$ počet rozdělovaných mandátů.
...
Pro kvótu $\frac{H}{N+1}$ se kvóta vejde do celkového počtu hlasů přesně $N+1$ krát, pokud se kvóta zvýší, už se do celkového počtu hlasů $N+1$ krát nevejde, vejde se tedy maximálně $N$, maximálně může být rozděleno $N$ mandátů.
...
Poté jsou vyřazeny veškeré hlasy pro strany které nesplnily podmínky pro zisk mandátů, tedy nedosáhly \uv{pětiprocentní hranice}.
...
Umělé rozdělení republiky na \uv{volební kraje} může také vést k politickým bojům o jejich hranice a počet.\footnote{Toto je vidět například v USA, kde se překreslování hranic volebních obvodů stalo důvodem vleklých politických i soudních bojů.}
